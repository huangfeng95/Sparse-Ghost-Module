...
class C3DSC(C3):
    # C3 module with DSCBottleneck()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(DSCBottleneck(c_, c_) for _ in range(n)))


class C3Fire(C3):
    # C3 module with OurBottleneck()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(FireBottleneck(c_, c_) for _ in range(n)))
...
class Fire(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        ...
        c_ = c2 // 4
        _c = c2 // 2
        self.conv1 = nn.Conv2d(c1, c_, 1, s, 0, bias=False)
        self.bn1 = nn.BatchNorm2d(c_)
        self.act1 = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.conv2 = nn.Conv2d(c_, _c, 1, 1, 0, bias=False)
        self.bn2 = nn.BatchNorm2d(_c)
        self.act2 = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.conv3 = nn.Conv2d(c_, _c, 3, 1, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(_c)
        self.act3 = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())

    def forward(self, x):
        x1 = self.act1(self.bn1(self.conv1(x)))
        x2 = self.act2(self.bn2(self.conv2(x1)))
        x3 = self.act3(self.bn3(self.conv3(x1)))
        out = torch.cat([x2, x3], 1)
        return out


class FireBottleneck(nn.Module):
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.fire1 = Fire(c1, c_, 1, 1)

        # Depth-wise convolution
        if self.stride > 1:
            self.conv_dw = nn.Conv2d(c_, c_, dw_kernel_size, stride=s,
                             padding=(dw_kernel_size-1)//2,
                             groups=c_, bias=False)
            self.bn_dw = nn.BatchNorm2d(c_)

        # Point-wise linear projection
        self.fire2 = Fire(c_, c2, act=False)
...
class DSC(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        super().__init__()
        self.conv1 = nn.Conv2d(c1, c1, k, s, k//2, groups=c1, bias=False)
...
class GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):  # ch_in, ch_out, kernel, stride, groups
        ...
        self.cv2 = Conv(c_, 3*c_, 5, 1, None, c_, act)
